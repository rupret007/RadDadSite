...
5. Run the protected workflow with `production_action=deploy` for the exact
   current `main` SHA.
6. Complete every command and browser check in
   [Production verification](#production-verification), including both domains
   and the exact public SHA.
7. Review the GitHub logs for unexpected environment dumps, PEM headers,
...
8. Confirm GitHub Pages and the ChatGPT Sites URL still work independently and
   were not reconfigured.
9. After a second reviewed SHA exists, perform the rollback drill below.
10. Complete the legacy-root quarantine procedure. Do not delete it.
11. Only after both owners accept the deployment, rollback, redeployment,
...
1. Set the repository variable `PROD_AUTO_DEPLOY_ENABLED=false`. This blocks
   future push-triggered deployment jobs; it does not retract a job that GitHub
   already admitted.
