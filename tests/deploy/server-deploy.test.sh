...
with tempfile.TemporaryDirectory() as temporary:
    ...
    (root / "assets").mkdir()
    (root / "assets" / "rad-dad-friends-guitars-growlers-2026-1122.webp").write_bytes(
        b"fixture-flyer-large"
    )
    (root / "assets" / "rad-dad-friends-guitars-growlers-2026-561.webp").write_bytes(
        b"fixture-flyer-small"
    )
    (root / "assets" / "rad-dad-friends-guitars-growlers-2026-full.png").write_bytes(
        b"fixture-flyer-full"
    )
    (root / "assets" / "the-middle-jimmy-eat-world-thumbnail.webp").write_bytes(
        b"fixture-thumbnail"
    )
    (root / "assets" / "rad-dad-friends-guitars-growlers-2026.ics").write_text(
        "BEGIN:VCALENDAR\r\nEND:VCALENDAR\r\n", encoding="utf-8"
    )
    (root / "assets" / "rad-dad-social-2026.png").write_bytes(b"fixture-social")
    (root / "assets" / "wildflower-2026-poster-720.webp").write_bytes(
        b"fixture-wildflower"
    )
