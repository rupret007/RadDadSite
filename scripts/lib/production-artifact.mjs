...
async function validateLocalReferences(artifactRoot) {
    const references = [];
    const html = await readFile(resolve(artifactRoot, 'index.html'), 'utf8');
    const css = await readFile(resolve(artifactRoot, 'styles.css'), 'utf8');
    ...
    for (const match of html.matchAll(singleUrlPattern)) {
        references.push({ reference: match[2], sourcePath: 'index.html' });
    }

    for (const match of html.matchAll(sourceSetPattern)) {
        for (const candidate of match[2].split(',')) {
            const reference = candidate.trim().split(/\s+/, 1)[0];
            references.push({ reference, sourcePath: 'index.html' });
        }
    }
